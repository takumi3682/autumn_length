...
仙台の年間平均気温の分布（ヒストグラム）は次の通り（2020年）
...
p1 <- plot_hist_single_year(df, 2020, station_name_e)
p1
```

5℃付近と24℃付近にピークを持つ2峰型のように見える。
期間を1991〜2020年の30年間として同様のグラフを描いてみる。

```{r}
p2 <- plot_hist_period(df, 1991, 2020, station_name_e)
p2
```
...
p_spring <- plot_kde_target_months(df, target_months = c(3, 4, 5), 
            legend_position = c(0.80, 0.75), title = FALSE)
p_summer <- plot_kde_target_months(df, target_months = c(6, 7, 8), 
            legend_position = c(0.20, 0.75), title = FALSE)
...
final_plot <- combined_plot + 
  ...
  # --- 各パネルの微調整 ---
  theme(
    plot.title = element_text(size = 10),      # 各パネル（春・夏など）のタイトル
